...
function stripIndentation(html) {
    ...
    var indentation = getMinimumIndentation(lines);

    for (var lineIndex in lines)
        lines[lineIndex] = lines[lineIndex].substr(indentation);

    lines.splice(-1, 1);
    return lines.join('\n');
}

function getMinimumIndentation(lines) {
    var tabCount = null;
    for (var lineIndex = 1; lineIndex < lines.length; lineIndex++) {
        for (var charIndex = 0; charIndex < lines[lineIndex].length; charIndex++) {
            if (lines[lineIndex][charIndex] != ' ') {
                if (tabCount === null || tabCount > charIndex)
                    tabCount = charIndex;
    ...
    for (var i = 0; i < dom.length; i++) {
        ...
        if (e.type == 'tag' || e.type == 'script' || e.type == 'style') {
            ...
            if (e.name == "pre") {
                ...
                e.children[0].data = stripIndentation(e.children[0].data);
                ...
                if (e.children && e.children.length > 0) {
                    if (e.children.length == 1 && (e.children[0].type == 'text' || e.children[0].type == 'comment')) {
                        if (e.children[0].type == 'text') {
                            ...
                            if (e.name == "pre" || e.name == "code") output += trim(txt);
